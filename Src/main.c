...
int length = 0 ;
int count_10ms=0 ;
int count_100ms=0 ;							// Verz�gerungen
int count_1s=0 ;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_TIM_Base_Start(&htim1) ;					// Timer f�r ADC Abfrage 
...
//	MY_FLASH_ReadN(0, w_steering, flasharray_length[0], DATA_TYPE_16);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		if(HAL_GPIO_ReadPin(Flash_start_GPIO_Port, Flash_start_Pin))			// �bertragung Start
		{	memory_start = 1 ;
		...
		if((bluebuffer[3]==8))	{
				// angle_sync = 1;
			sync_allowed=1;
			if(HAL_GPIO_ReadPin(photodiode1_GPIO_Port, photodiode1_Pin))			// �bertragung Start
				{	photodiode = 1;	}
			else{
				photodiode = 0;
			}
...
//		}
		...
		HAL_ADC_Start(&hadc1);		
		if(HAL_ADC_PollForConversion(&hadc1,10) == HAL_OK)	{			// Single conversion f�r Distanz
			adcvaluek = HAL_ADC_GetValue(&hadc1);
			sensork=2*(2076.0/(adcvaluek-11));
		}	
		HAL_ADC_Start(&hadc2);		
		if(HAL_ADC_PollForConversion(&hadc2,10) == HAL_OK)	{			// Single conversion f�r Winkel
				...
				if(steering_conv >= angleconv_sync){
					steering_trailer = map(steering_conv, angleconv_sync, angleconv_sync-90, 0, 90);					// Adcvals werden mit gas und lenken gemapt, sprich, umgewandelt in gew�nschte werte
				}
				if(steering_conv < angleconv_sync){
		...
		if(count_10ms%2==0)	{ //Pucher 20ms begin
			if(setval_memory_storage==1){
				if(memory_start==1){
					curve_was_taken = 1;
					velocity[icom]=	spinstrans ;		// �bergabe der Sensorwerte
...
//					HAL_UART_Transmit(&huart3,velocityASCII,sizeof(velocityASCII),1);		// �bertragung �ber UART
//					HAL_UART_Transmit(&huart3,steeringASCII,sizeof(steeringASCII),1);
				...
				if((memory_start==2)||(icom>=9999)){
					...
					steering[icom] = lenken ;
					thrust[icom] = 0 ;
					flasharray_length[0] = icom+1;	//memory_start= 0 einf�gen?
					flash_index = icom;
				}
			}
...
//				}
		...
		*/
		...
		//Berechnung der Lenk-,Gas-Werte und Schalten der zugeh�rigen PWM-GPIOs
		//Lenkung - Steering
